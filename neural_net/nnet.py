...
class NN:
    ...
    def Get_Energy(self, x):
        ...
        # print("W",np.shape(self.W))
        # print("x", np.shape(x))
        # print("matmul - W*x",np.shape(np.matmul(self.W, x)))
        # print("B1: ",np.shape(self.B1))
        # print("z2",np.shape(z2))
        # print("o",np.shape(o))
        # print("s",np.shape(s))
        ...
        if not self.outer_relu:
            ...
            for i in range(N):
                for j in range(N):
                    if(dLdOut[i,j]!=0):
                        total_supposed+=1
                    if dLdOut[i, j] != 0 and (featVMat[i][j] is not None) and type(featVMat[i][j])!=int:
                        
                        batch_size += 1
                        # print(i,j)
                        x = featVMat[i][j][0:1500]
                        (z2, a2, s) = self.Forward_Prop(np.reshape(x,(1500,1)))
                        # print(a2.transpose())
                        # print('o')
                        ...
                        # print("dRelu",np.shape(dRelu))
                        # print("x",np.shape(x))
                        # print("self.r1",np.shape(self.r1))
                        dLdW += (dLdOut[i, j])*np.matmul((self.U*dRelu), (np.reshape(x,(1500,1))*self.r1).transpose())

                        dLdB1 += dLdOut[i, j]*np.matmul(self.U.transpose(), dRelu)
            print("Batch_size: ",batch_size,"Total_supposed :",total_supposed)
            if batch_size > 0:
