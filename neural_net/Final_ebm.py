...
class Trainer:
	...
	def Test(self, sentenceObj, dcsObj, dsbz2_name, _dump = False, _outFile = None):
		...
		for source in range(len(nodelist)):
			(mst_nodes, mst_adj_graph, mb) = clique(nodelist, WScalarMat, conflicts_Dict, source) ## call "least_edge_first" instead of clique
			# print('.', end = '')
			cst = ''
			for i in mb:
				if(i):
...
#               format(word_match, lemma_match, len(dcsLemmas), len(nodelist)))
	...
	def Train(self, featVMat, gold_edges_mask, edges_to_consider, node_list, total_nodes,mode ='nearest_neighbour',_debug = True):
		...
		if(mode=='least_edge_first'):
			...
			mst_adj_graph, patha = least_edge_first(WScalarMat,edges_to_consider, total_nodes) # T_X
			#print("Z")
			#mst_adj_graph = clique(WScalarMat,edges_to_consider,node_list, total_nodes)
			# print("Predicted Edges")
			# print(mst_adj_graph)
			#print("before np.sum")
			#print(mst_adj_graph)
			#print(np.ndim(mst_adj_graph))
...
#	try:
		...
		# t_file = '285222'
		d = node_dict[t_file]
		with bz2.open(str(feat_dir)+str(t_file.replace('.graphml',''))+".bz2", 'rb') as f:
			featVMat = pickle.load(f)  ## this is feature Matrix with arrays

		G = read_graphml('After_graphml/'+str(t_file))
...
def main():
	...
	nodecsvfile = 'gtnodeorder_119k.csv'
	...
	node_dict = dict()
	with open("gtnodeorder_119k.csv", 'r') as csvfile:
		rd = csv.reader(csvfile)
		for row in rd:
			node_dict[row[0] + '.graphml'] = row[1:] 

	# df = pd.read_csv('train_files.csv')

	# print(df.columns)

	train_files = sorted(os.listdir('After_graphml'))[:80000]
	#print(train_files)
	## Call train function with appropriate list of files and also the directory to pick the files from
	train_generator(train_files, node_dict, feat_dir)
