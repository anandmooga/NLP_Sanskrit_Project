...
def Get_W_Scalar_Matrix_from_FeatVect_Matrix(featVMat, _neuralnet):
    WScalarMat = np.zeros((len(featVMat),len(featVMat)))
    if not _neuralnet.outer_relu:        
        ...
        for i in range(len(featVMat)):
            for j in range(len(featVMat[i])):
                if featVMat[i][j] is None or featVMat[i][j] is 0:
                    pass
                else:
